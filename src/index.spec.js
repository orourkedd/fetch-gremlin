...
test('should fetch', async () => {
  const fetch = buildFetch({ badResponse: 0, immediatelyFailure: 0 })
...
test('should fail immediately', async () => {
  const fetch = buildFetch({ badResponse: 0, immediatelyFailure: 1 })
  try {
...
test('should respond with a 500', async () => {
  const fetch = buildFetch({ badResponse: 1, immediatelyFailure: 0 })
  const response = await fetch('https://swapi.co/api/people/1')
